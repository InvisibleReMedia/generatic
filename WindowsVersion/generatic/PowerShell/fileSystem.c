...
#define CREATEONEDIR_CMD "powershell.exe -f PowerShell\\createOneDirectory.ps1 \"\" \"\""
#define CREATEDIR_CMD "powershell.exe -f PowerShell\\createDirectory.ps1 \"\" \"\""
#define CREATEFILE_CMD "powershell.exe -f PowerShell\\createFile.ps1 \"\" \"\""
#define ISPATHEXIST_CMD "powershell.exe -f PowerShell\\testPath.ps1 \"\""
#define ISDIREXIST_CMD "powershell.exe -f PowerShell\\testDir.ps1 \"\" \"\""
#define ISFILEEXIST_CMD "powershell.exe -f PowerShell\\testFile.ps1 \"\" \"\""
#define DELETEDIR_CMD "powershell.exe -f PowerShell\\deleteDir.ps1 \"\" \"\""
#define READFILE_CMD "powershell.exe -f PowerShell\\readFile.ps1 \"\" \"\""
#define QUOTEVALUE	'"'
...
void createOneDirectory(wchar_t* path, wchar_t* dir) {
	...
	long lenText;
	if (lenPath == 0)
		lenText = strlen(CREATEONEDIR_CMD);
	else
		lenText = strlen(CREATEDIR_CMD);
    
    if (lenDir + lenPath + lenText < 255) {
		char cmd[256];
		if (lenPath == 0)
			strcpy(cmd, CREATEONEDIR_CMD);
		else
			strcpy(cmd, CREATEDIR_CMD);
		char cdir[256], rdir[256];
        memset(cdir, 0, 256);
        memset(rdir, 0, 256);
        for(int index = 0; index < lenPath; ++index) {
		...
		char* res;
		if (lenPath == 0)
			res = combine(cmd, rdir);
		else
			res = combine2(cmd, cdir, rdir);
        system(res);
...
/**
 **   Create all successives directories
 **/
void createAllDirectories(wchar_t* dir) {
...
void createFile(wchar_t* path, wchar_t* file) {
	...
	/** ensures all recursive directory are created **/
